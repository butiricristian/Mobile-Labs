  ...
  constructor(props){
    ...
    this.auth.onAuthStateChanged((user) => {
      if(user){
        ...
        this.database = global.firebaseApp.database().ref().child('users').child(user.uid).child("premium");
        this.database.on('value', (snap) => {
          this.props.navigation.navigate('BookList', {uid: user.uid, isPremium: snap});
        })
      }
      else{
    ...
    return (
      <View style={styles.container}>
        <View style={{flexDirection: 'column', flex: 0.2, width: '80%'}}>
          ...
          <TextInput caretHidden = {true} style = {{flex: 0.8, height: 40}} onChangeText={(password) => this.setState({password})}/>
